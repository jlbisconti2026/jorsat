# Indice

[Introduccion](#introduccion)
[Extraccion de pullsecret hacia archivo dockerconfigjson](#extraccion-de-pullsecret-hacia-archivo-dockerconfigjson)
[Verificamos existencia de parametro cloud.openshift.com](#verificamos-existencia-de-parametro-cloudopenshiftcom)
[Editamos archivo  (dockerconfigjson](#editamos-archivo--dockerconfigjson)
[Verificamos que ya no tenga el parametro](#verificamos-que-ya-no-tenga-el-parametro)
[Aplicamos nuevo pull secret](#aplicamos-nuevo-pull-secret)





## Introduccion

A contiuacion vemos los pasos para deshabilitar insigths tanto en OCP como en OKD. Este paso se incluye dentro del dia 2 posterior a la instalacion del cluster.


## Extraccion de pullsecret hacia archivo dockerconfigjson

```bash
[root@labarq01adl bkp-pull-secret]# oc extract secret/pull-secret -n openshift-config --to=.dockerconfigjson
```

## Verificamos existencia de parametro cloud.openshift.com

```bash
[root@labarq01adl bkp-pull-secret]# cat .dockerconfigjson
```

{"auths":{"cloud.openshift.com":{"auth":"b3BlbnNoaWZ0LXJlbGVhc2UtZGV2K29jbV9hY2Nlc3NfM2IxMzk4ZjQ2MWE5NDE2Mzg4Njk0YWY3ZDNlNmU5MTU6WDdYUVcwTjNYQ0k0MUo2U0hGR1YySEdWN1NFUkg0MkwwVVQ2OElRM0szOVpaQkhJTU9SSlQ3NUpZRERUN1U4RA==","email":"ignacio.bellucci@claro.com.ar"},"quay.io":{"auth":"b3BlbnNoaWZ0LXJlbGVhc2UtZGV2K29jbV9hY2Nlc3NfM2IxMzk4ZjQ2MWE5NDE2Mzg4Njk0YWY3ZDNlNmU5MTU6WDdYUVcwTjNYQ0k0MUo2U0hGR1YySEdWN1NFUkg0MkwwVVQ2OElRM0szOVpaQkhJTU9SSlQ3NUpZRERUN1U4RA==","email":"ignacio.bellucci@claro.com.ar"},"registry.connect.redhat.com":{"auth":"","email":"ignacio.bellucci@claro.com.ar"},"registry.redhat.io":{"auth":"","email":"ignacio.bellucci@claro.com.ar"}}}


## Editamos archivo  dockerconfigjson

```bash
[root@labarq01adl bkp-pull-secret]# vi .dockerconfigjson
```
 
Removemos parametro cloud.openshift.com


## Verificamos que ya no tenga el parametro
 
```bash
[root@labarq01adl bkp-pull-secret]# cat .dockerconfigjson
```

{"auths":{"":{"auth":"b3BlbnNoaWZ0LXJlbGVhc2UtZGV2K29jbV9hY2Nlc3NfM2IxMzk4ZjQ2MWE5NDE2Mzg4Njk0YWY3ZDNlNmU5MTU6WDdYUVcwTjNYQ0k0MUo2U0hGR1YySEdWN1NFUkg0MkwwVVQ2OElRM0szOVpaQkhJTU9SSlQ3NUpZRERUN1U4RA==","email":"ignacio.bellucci@claro.com.ar"},"quay.io":{"auth":"b3BlbnNoaWZ0LXJlbGVhc2UtZGV2K29jbV9hY2Nlc3NfM2IxMzk4ZjQ2MWE5NDE2Mzg4Njk0YWY3ZDNlNmU5MTU6WDdYUVcwTjNYQ0k0MUo2U0hGR1YySEd

.....


## Aplicamos nuevo pull secret

```bash
[root@labarq01adl bkp-pull-secret]# oc set data secret/pull-secret -n openshift-config --from-file=.dockerconfigjson=/root/bkp-pull-secret/.dockerconfigjson
```
Veremos el mensaje :
secret/pull-secret data updated
